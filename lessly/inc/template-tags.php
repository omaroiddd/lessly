...
if ( ! function_exists( 'lessly_posted_by' ) ) :
	/**
	 * Prints HTML with meta information about theme author.
	 */
	function lessly_posted_by() {
		printf(
		/* translators: 1: posted by label, only visible to screen readers. 2: author link. 3: post author. */
			'<span class="sr-only">%1$s</span><span class="author vcard"><a class="url fn n" href="%2$s">%3$s</a></span>',
			esc_html__( 'Posted by', 'lessly' ),
...
if ( ! function_exists( 'lessly_comment_count' ) ) :
	/**
	 * Prints HTML with the comment count for the current post.
	 */
	function lessly_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			/* translators: %s: Name of current post. Only visible to screen readers. */
			comments_popup_link( sprintf( __( 'Leave a comment<span class="sr-only"> on %s</span>', 'lessly' ), get_the_title() ) );
		}
	}
endif;

if ( ! function_exists( 'lessly_entry_meta' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 * This template tag is used in the entry header.
	 */
	function lessly_entry_meta() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( __( ', ', 'lessly' ) );
			if ( $categories_list ) {
				printf(
				/* translators: 1: posted in label, only visible to screen readers. 2: list of categories. */
					'<span><span class="sr-only">%1$s</span>%2$s</span>',
					esc_html__( 'Posted in', 'lessly' ),
					$categories_list // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				);
			}

			/* translators: used between list items, there is a space after the comma. */
			$tags_list = get_the_tag_list( '', __( ', ', 'lessly' ) );
			if ( $tags_list ) {
				printf(
				/* translators: 1: tags label, only visible to screen readers. 2: list of tags. */
					'<span><span class="sr-only">%1$s</span>%2$s</span>',
					esc_html__( 'Tags:', 'lessly' ),
		...
		edit_post_link(
			sprintf(
				wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers. */
					__( 'Edit <span class="sr-only">%s</span>', 'lessly' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'lessly_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function lessly_entry_footer() {
...
if ( ! function_exists( 'lessly_comment_avatar' ) ) :
	/**
	 ...
	 */
	function lessly_get_user_avatar_markup( $id_or_email = null ) {

		if ( ! isset( $id_or_email ) ) {
			$id_or_email = get_current_user_id();
		}

		return sprintf( '<div class="vcard">%s</div>', get_avatar( $id_or_email, lessly_get_avatar_size() ) );
	}
endif;

if ( ! function_exists( 'lessly_discussion_avatars_list' ) ) :
	/**
	 * Displays a list of avatars involved in a discussion for a given post.
	 *
	 * @param array $comment_authors Comment authors to list as avatars.
	 */
	function lessly_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				lessly_get_user_avatar_markup( $id_or_email ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
		echo '</ol>', "\n";
	}
...
if ( ! function_exists( 'lessly_the_posts_navigation' ) ) :
	/**
	 * Wraps `the_posts_pagination` for use throughout the theme.
	 */
	function lessly_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => __( 'Newer posts', 'lessly' ),
				'next_text' => __( 'Older posts', 'lessly' ),
			)
		);
	}
endif;

if ( ! function_exists( 'lessly_content_class' ) ) :
	/**
	 ...
	 * LESSLY_TYPOGRAPHY_CLASSES constant.)
	 *
	 ...
	 */
	function lessly_content_class( $classes = '' ) {
		$all_classes = array( $classes, LESSLY_TYPOGRAPHY_CLASSES );
